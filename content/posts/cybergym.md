...
因此，我们定义 **Constraint-Closed Dataflow Segment（CCDS，约束闭合数据流段）**：一个 CCDS 是完整数据流上的一段连续区域，**并包含使数据能够从该段入口传播到该段出口所需要的全部输入字节及其依赖关系**。如果某个条件依赖其他片段中的输入字节，则相关片段必须合并，直到这种跨片段依赖消失。
